  ...
  return browser.tabs.executeScript(tid, {
    ...
    code: `(function () {
		...
		if ('gain' in newSettings) {
			el.xSoundFixerGain.gain.value = Math.pow(2, newSettings.gain) - 1
		}
    ...
    for (const [fid, els] of frameMap) {
      for (const [elid, el] of els) {
        ...
        gain.addEventListener("input", function () {
          ...
          this.parentElement.querySelector(".element-gain-num").value =
            "" + this.value;
        });
      ...
      function applyPan(value) {
        for (const [fid, els] of frameMap) {
          for (const [elid, el] of els) {
            ...
            epan.value = value;
            epan.parentElement.querySelector(".element-pan-num").value =
              "" + value;
          }
        }
        pan.value = value;
